...
static void aes_increase_counter(uint8_t *counter) {
  ...
  for (int block_i = 0; block_i < (FS_BLOCK_SIZE / AES_BLOCK_SIZE); block_i++) {
    for (uint8_t* cur_pos = counter + (block_i * AES_BLOCK_SIZE) + 14; 
         cur_pos >= counter + (block_i * AES_BLOCK_SIZE); 
         cur_pos--) {
      (*cur_pos) += 1;
      if (*cur_pos != 0) {
        break;
 ...
 * @return void* 
 */
inline void *aes_ctr_encdec_bbb(aes_state_t *state, int fd, const uint8_t *src, uint32_t len, uint32_t read_bytes) {
  int nb;
...
inline void *aes_ctr_encdec_ex(aes_state_t *state, int fd, const uint8_t *src, uint32_t len, uint32_t read_bytes, uint8_t *dst) {
  int nb;
  uint32_t in_block_offset;
  // uint32_t num_remain_bytes = len;
  ...
  uint32_t bytes_to_next_aligned = (FS_BLOCK_SIZE - in_block_offset) % FS_BLOCK_SIZE;
  ...
  while(num_blocks > 0) {
    ...
    curr_src += FS_BLOCK_SIZE; curr_dest += FS_BLOCK_SIZE;
...
void *aes_gcm_encdec(aes_state_t *state, int fd, const uint8_t *src, uint32_t len, uint32_t read_bytes, uint8_t *mac) {
  return NULL;
}